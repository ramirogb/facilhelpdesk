...
class POP3 {
    ...
    */
    function get_mail( $msg_number, $qmailer = FALSE )
    {
        if(!$this->socket)
        {
            $this->error = "POP3 get_mail() - Error: No connection availble.";
        ...
        if ($qmailer == TRUE)
	{	if(substr($response,0,1) != '.') 
		{$this->error = "POP3 get_mail() - Error: ".$response;return FALSE;		}
	}
	else 
	{
		if(substr($response,0,3) != "+OK") 
        ...
        //while(!eregi("^\.\r\n",$response))
		while(!preg_match("^\.\r\n",$response)) //while if don't find the "end", outputs it
        {
            $output[$i] = $response;
            $i++;
             $response = $this->_getnextstring();
    ...
    */
    ...
    function delete_mail($msg_number = "0")
    {
	   ...
	   //echo 'D===============<BR>';
        if(!$this->_logging($cmd)) return FALSE;
        if(!$this->_putline($cmd)) return FALSE;
        echo $response = $this->_getnextstring();
        if(!$this->_logging($response)) return FALSE;
        if(substr($response,0,1) != "+"){
...
function simple_list(){
        ...
        if(!$this->_checkstate("stats")) return FALSE;
        $cmd = "STAT"; //devuelve el numero de mails y el tamaño
        if(!$this->_logging($cmd)) return FALSE;
        if(!$this->_putline($cmd)) return FALSE;
