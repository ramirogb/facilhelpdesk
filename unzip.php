...
//		echo $this->fileName;
	...
	Function unzipAll($targetDir=false, $baseDir="", $maintainStructure=true, $applyChmod=777){ 
		//if($targetDir == false)
			// $targetDir = dirname(__FILE__); //."/";
			$targetDir = ''; //aqui cambie yo
			//echo $targetDir; i deleted targetdit
			$_SESSION['reporte']='';
			$_SESSION['reporte'] ='These files were copied/overwriten:'.'<BR>';
		...
		if(sizeof($lista)) foreach($lista as $fileName=>$trash){
			...
			$maintainStructure?
				...
				chmod($fileName,0644); ////was 777 but the server crash
				$_SESSION['reporte'] .=basename($fileName).'<BR>';
		}
	}
...
$filename='helpdesk.zip';
//$filename=  $uploadpath.basename($_SESSION['gthy']);
$zip = new dUnzip2($filename);
$zip->getList();
$zip->unzipAll('');

$filename=  './install.php'; //$website.'/'.$folder.'/install.php'; //uploadpath.basename($_SESSION['gthy']);
$jump_here="Location: ".$filename;	      
 header($jump_here); 
