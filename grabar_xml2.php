...
$r = $doc->createElement( "searchresult" );
  $doc->appendChild( $r );
...
//echo $suma;
	 if ( ($suma==true) and  ($lengua==$lang_obj) )
	  	{
		$tickets_question= (xmlentities($tickets_question));
		$titulo=( xmlentities($titulo));
...
$b = $doc->createElement( "document" );
  
  $title = $doc->createElement( "title" );

  $title->appendChild($doc->createTextNode(  htmlentities(  $titulo)   ));
  $b->appendChild( $title);
  
  $url = $doc->createElement( "url" );
  $laurl=' ';
  $url->appendChild( $doc->createTextNode( $laurl ));
  $b->appendChild( $url);
  
  $snippet = $doc->createElement( "snippet" );
  $tickets_question='BAS';
 $tickets_question= iconv("utf-8", "utf-8//IGNORE", $tickets_question);
//$tickets_question = utf8_encode($tickets_question);

  $snippet->appendChild(  $doc->createTextNode( $tickets_question ));
  $b->appendChild( $snippet);
