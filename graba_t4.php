...
$ticketid=$_GET['ticketid'];
        ...
        if ($_GET['action']<>'home'){ $resultxx5 = mysql_query( $query556 );}$cerrare=false;
...
{$cerrare=true;}
			...
			$fila84=mysql_fetch_assoc($result84);
 ...
 if (     isset($_POST['ticket'])   )  FOREACH ( $_POST['ticket'] AS $ticketid)
{
        ...
        { $resultxx5 = mysql_query( $queryas ); }
		   ...
		   $result3x4 = mysql_query($query3x4);   	       
			
		   $query3x3x = "SELECT tickets_id FROM   tickets_tickets WHERE tickets_child=$ticketid";
 	       $resultabc = mysql_query($query3x3x);
		   $rowchi=mysql_fetch_row( $resultabc);  
		   do
		   {
			//deleting of the main ticket	
		   $padre=$rowchi[0]; $query3x4 = "DELETE FROM   tickets_atach WHERE tickets_id=$padre ";  $result3x4 = mysql_query($query3x4);
		   }
			while ($rowchi=mysql_fetch_row( $resultabc)) ; 				      
			//files deleting end	   
...
{//ONLY CHANGE DEPARTAMENT
   ...
   if ($emailasigned=='TRUE' ) //email to the assigned staff member and his supervisor,it comes from configuration
                  {				
                   if ($was_assigned_ticket== true ) //1 mail to the staff
					{ 				
			        	...
			        	$result_xz = mysql_query( $un_sql );
						$filaxz = mysql_fetch_assoc($result_xz); //this notificates that he has  a new ticket
								if ( $sendhtml=='TRUE' )
                				{	$separator='<BR>';}
     							else
				...
				$un_sql="	SELECT email,name FROM users where id='$supervisoremail' ";
				  $result_xz = mysql_query( $un_sql );
					$filaxz = mysql_fetch_assoc($result_xz);
...
// ticket_admin, contiene quien manda, username quien es el receptor 
			...
			{
			   $id_ticket=mysql_insert_id();//if ticket child=0 entonces es un ticket nuevo entonces seteo unread_user
           $number_of_response='';
		   $el_id=$_GET['ticketid'];		   
			//echo 'punto 00';
          if ( $el_id <>0) 
		   ...
		   $resultxx5 = mysql_query( $queryzzz1);
		   }
			...
			$result_un = mysql_query( $un_sql );
			$ggg= mysql_fetch_array($result_un);
	...
	// EMAIL ADMINISTRATOR THE TICKET NOTIFICATION				
               ...
               $rate=$to_rate_responses.$separator.'<a href="'  .$la_url. '">'.'Link:'.'</a>'.$separator;			   
				...
				$filaxx=mysql_fetch_assoc($resultxx);				
				...
				*/				
	            ...
	            $watch_you_ticket	='<a href="'.$watch_you_ticket0."\">$openticket</a>".
                 ...
                 $resultxx5 = mysql_query( $query556x );
				 $thekey=mysql_fetch_row($resultxx5);				 
				 $watch_you_ticket0=$siteurl.'tickets.php'
	...
	// LOOP THROUGH THE PREVIOUS MESSAGES AND ADD DATA REGARDING QUESTION TIME AND ATTACHMENT
					...
					if ( $sendhtml=='TRUE' ){ $dataText=nl2br($dataText);}
		             ...
		             $the_email=mysql_fetch_row($resultcc);
			  ...
			  , closed_by='".$_SESSION['xcv_userna']."' WHERE id   = '".$ticketid."'";			
			    IF (!mysql_query($query)) { $message='Was not posible to change status of ticket';  }              
			    }	  
...
IF (  ($_GET['action']=='users_dep_edit') )
{
        ...
        $result = mysql_query($query_users_staff);
        }
        else
       {$last_msg='Denied not Admin or Demo';}
...
for ($a=1; $a<=$contador;$a +=1 )
{
           ...
           if ($_POST[$a]==true)
           {
			 ...
			 user='$user_id',departament='$depax',departament_visible='$depax'  ";
			   if (is_admin()=== true)
			   {
				$result5ins = mysql_query($query_inse5ins);
				}
				else
				$last_msg='Denied not Admin or Demo';
           }
...
//============OPEN TICKET========================
	...
	$el_ticket=$_GET['ticketid'];
	 ...
	 $result1  = mysql_query($query44);
	 $rett=mysql_fetch_row($result1);
	 if  ( $rett[0]=='')
	 {	 
...
//============CREATING REPORTS==================
	...
	$totaltickets1 = mysql_num_rows($result1);	
	...
	$totaltickets2 = mysql_num_rows($result2);
	...
	$totaltickets3 = mysql_num_rows($result3);	
	$report1=true;
	$authz='TRUE';
	include('./includes/top2.php');
	include('./includes/reports.php');
...
//===Banned users=============
	...
	$row_Recordsetx = mysql_fetch_assoc($resulta);
	       $your_data = "";
         do {
 ...
 while ($row_Recordsetx = mysql_fetch_assoc($resulta));	
	     $ghgh='report.txt';
         touch($ghgh);
	...
	$result1  = mysql_query($query1);	
	  $query2="SELECT ca.tickets_categories_name as nom, avg(a), avg(b),
		  ...
		  and tickets_poll.id=ti.tickets_id		  
		   group by nom ";	 
$result2_rating  = mysql_query($query2);
...
$result3_rating  = mysql_query($query3);
	 ...
	 $query4="SELECT ti.tickets_username, avg(a), avg(b),
		...
		";		  
         $result4_rating  = mysql_query($query4);$authz='TRUE';
	include('./includes/top2.php');
	include('./includes/reports.php');
	include('includes/bottom.php');
	exit;		
...
if ( $_GET['action']=='deletelog2')	
	...
	{
    $report20=true;	$query1="SELECT * FROM error_log ORDER BY timestamp DESC LIMIT 600";$result1  = mysql_query($query1);$authz='TRUE';
...
$result_permits  = mysql_query($query_permits);$fil='';
$permisos='(';
do
{
                     if ($fil[0] <>'')
...
while ( $fil=mysql_fetch_row($result_permits) );
	  ...
	  //si es moderador comparamos sus permisos con los que tienen los tickets de la tabla tickes  
        ...
        (  a.tickets_category in $permisos )
	     ...
	     ((d.assigned_to='$nombrer') and (d.assigned='1')) or   (   (d.assigned != '1') or (d.assigned is null)  )
		   ...
		   $addon  = '';
   			$result_preliminar   = mysql_query($query);
			 $total_ticket_search_in_parent = mysql_num_rows($result_preliminar);			
			if ($total_ticket_search_in_parent>0)//If exists any results
			{  //######3

		   	//$tickets_childs='(12)';
           $filvvv=mysql_fetch_assoc($result_preliminar);
         ...
         while ( $filvvv=mysql_fetch_assoc($result_preliminar) );
...
//echo $q2;	
 $query = sprintf("%s LIMIT %d, %d",$query, $startRow_Recordset1, $maxRows_Recordset1); 				
			$result = mysql_query($query);
	$totalRows_contenido = mysql_num_rows($result);								
$q3=time();
 ?>
 <?php 
function set_class()
